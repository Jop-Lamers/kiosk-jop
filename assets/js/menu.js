document.addEventListener("DOMContentLoaded", () => {
  ...
  window.runFilter = function(showSkeletons = true) {
    ...
    const process = () => {
      ...
      products.forEach((p) => {
        ...
        const pName = (p.getAttribute('data-name-' + lang.toLowerCase()) || "").toLowerCase();
        const pDesc = (p.getAttribute('data-desc-' + lang.toLowerCase()) || "").toLowerCase();
        const pPopular = p.getAttribute('data-popular');
        
        const catMatch = (window.currentCat === 'all' || pCat == window.currentCat);
      ...
      if (visibleCount === 0) {
        ...
        noResultMsg.innerText = lang === 'EN' ? 'No products found matching your search.' : 'Geen gerechten gevonden voor deze zoekopdracht.';
  ...
  document.querySelectorAll('.cat-btn').forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (nameEl) nameEl.innerText = btn.getAttribute('data-' + lang.toLowerCase());
      if (descEl) descEl.innerText = btn.getAttribute('data-desc-' + lang.toLowerCase());
  ...
  const customizationRules = [
    { keywords: ['avocado'], options: [{ nl: 'Extra Avocado', en: 'Extra Avocado', price: 1.50 }, { nl: 'Geen Avocado', en: 'No Avocado', price: 0 }] },
    { keywords: ['feta', 'kaas', 'cheese', 'halloumi'], options: [{ nl: 'Extra Kaas', en: 'Extra Cheese', price: 1.25 }, { nl: 'Geen Kaas', en: 'No Cheese', price: 0 }] },
    { keywords: ['ei', 'egg', 'roerei'], options: [{ nl: 'Extra Ei', en: 'Extra Egg', price: 1.00 }, { nl: 'Geen Ei', en: 'No Egg', price: 0 }] },
    { keywords: ['spinazie', 'spinach'], options: [{ nl: 'Extra Spinazie', en: 'Extra Spinach', price: 0.50 }, { nl: 'Geen Spinazie', en: 'No Spinach', price: 0 }] },
    { keywords: ['tofu'], options: [{ nl: 'Extra Tofu', en: 'Extra Tofu', price: 2.00 }, { nl: 'Geen Tofu', en: 'No Tofu', price: 0 }] },
    { keywords: ['tempeh'], options: [{ nl: 'Extra Tempeh', en: 'Extra Tempeh', price: 2.00 }, { nl: 'Geen Tempeh', en: 'No Tempeh', price: 0 }] },
    { keywords: ['falafel'], options: [{ nl: 'Extra Falafel', en: 'Extra Falafel', price: 1.50 }, { nl: 'Geen Falafel', en: 'No Falafel', price: 0 }] },
    { keywords: ['granola', 'haver', 'oat'], options: [{ nl: 'Extra Granola/Haver', en: 'Extra Granola/Oats', price: 0.75 }, { nl: 'Geen Granola', en: 'No Granola', price: 0 }] },
    { keywords: ['fruit', 'bes', 'berry', 'aardbei', 'strawberry', 'banaan', 'banana'], options: [{ nl: 'Extra Fruit', en: 'Extra Fruit', price: 1.25 }, { nl: 'Fruit Apart', en: 'Fruit on the side', price: 0 }] },
    { keywords: ['broccoli', 'groente', 'veggie', 'edamame', 'spinazie', 'spinach'], options: [{ nl: 'Extra Groenten', en: 'Extra Veggies', price: 0.75 }, { nl: 'Geen Groenten', en: 'No Veggies', price: 0 }] },
    { keywords: ['ui', 'uien', 'onion'], options: [{ nl: 'Geen Ui', en: 'No Onion', price: 0 }, { nl: 'Extra Ui', en: 'Extra Onion', price: 0.30 }] },
    { keywords: ['hummus'], options: [{ nl: 'Extra Hummus', en: 'Extra Hummus', price: 0.75 }, { nl: 'Geen Hummus', en: 'No Hummus', price: 0 }] },
    { keywords: ['matcha'], options: [{ nl: 'Extra Shot Matcha', en: 'Extra Matcha Shot', price: 1.00 }] },
    { keywords: ['smoothie'], options: [{ nl: 'Extra Proteïne', en: 'Extra Protein', price: 1.50 }, { nl: 'Geen Suiker', en: 'No Sugar', price: 0 }] },
    { keywords: ['kikkererwten', 'chickpea'], options: [{ nl: 'Extra Kikkererwten', en: 'Extra Chickpeas', price: 0.75 }] },
    { keywords: ['pindakaas', 'peanut'], options: [{ nl: 'Extra Pindakaas', en: 'Extra Peanut Butter', price: 0.75 }] },
    { keywords: ['quinoa', 'rijst', 'rice'], options: [{ nl: 'Extra Basis', en: 'Extra Base', price: 1.00 }] }
  ];
  ...
  window.openCustomizer = function(id) {
    ...
    upsellActive = false;
    currentUpsell = currentProduct.cross_sell_id ? window.allProducts.find(p => p.product_id == currentProduct.cross_sell_id) : null;

    const lang = localStorage.getItem('kiosk_lang') || 'NL';
    
    // Set Main Info
    document.getElementById('modal-title').innerText = currentProduct['name' + (lang === 'EN' ? '_en' : '')] || currentProduct.name;
    document.getElementById('modal-desc').innerText = currentProduct['description' + (lang === 'EN' ? '_en' : '')] || currentProduct.description;
    ...
    const combinedText = (currentProduct.name + ' ' + (currentProduct.description || '') + ' ' + (currentProduct.name_en || '') + ' ' + (currentProduct.description_en || '')).toLowerCase();
    
    let extrasCount = 0;
    let wishesCount = 0;

    // Detect if product is Savory or Sweet based on category
    const catName = (window.allCategories && window.allCategories[currentProduct.category_id]) || '';
    const isSweetCat = ['Breakfast', 'Drinks', 'Ontbijt', 'Dranken', 'Smoothies'].includes(catName);
    const isSavory = !isSweetCat || combinedText.includes('tofu') || combinedText.includes('falafel') || combinedText.includes('wrap') || combinedText.includes('toastie') || combinedText.includes('burger');
    ...
    // Update Titles
    document.getElementById('modal-extras-title').innerText = lang === 'EN' ? "Extra's" : "Extra's";
    document.getElementById('modal-wishes-title').innerText = lang === 'EN' ? "Wishes" : "Wensen";
    document.getElementById('modal-confirm-btn').innerText = lang === 'EN' ? "Add to Order" : "Toevoegen aan bestelling";
    
    // Setup Upsell
    const upsellBox = document.getElementById('upsell-container');
    if (currentUpsell) {
      upsellBox.classList.remove('hidden');
      document.getElementById('upsell-name').innerText = currentUpsell['name' + (lang === 'EN' ? '_en' : '')] || currentUpsell.name;
      document.getElementById('upsell-price').innerText = '€' + parseFloat(currentUpsell.price).toFixed(2);
      document.getElementById('upsell-img').src = window.productImages[currentUpsell.product_id];
  ...
  function showUpsellScreen() {
    ...
    const drinks = window.allProducts.filter(p => p.category_id == 6 || p.category_id == 7).slice(0, 4);
    const snacks = window.allProducts.filter(p => p.category_id == 5).slice(0, 4);
    
    // Shuffle and pick 2
    const pool = [...drinks, ...snacks].sort(() => 0.5 - Math.random()).slice(0, 2);
    ...
    // Update labels
    document.getElementById('upsell-view-title').innerText = lang === 'EN' ? "Nice to add?" : "Lekker voor erbij?";
    document.getElementById('upsell-skip-btn').innerText = lang === 'EN' ? "No thanks, continue" : "Nee bedankt, verder gaan";
  }
